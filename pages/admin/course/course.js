...
const CourseSection = ({isUserLoggedIn,isAdmin}) => {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`/api/course/deletecourse?id=${id}`, { method: "DELETE" });
      if (res.ok) {
              ...
              <div key={course._id} className="bg-white rounded-lg shadow-lg">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-4 flex justify-between">
                      ...
                      <button  onClick={() => router.push(`/admin/course/course-update-form?courseId=${course._id}`)} className="px-4 py-2 text-white bg-blue-500 hover:bg-blue-600 rounded">
                        Update
                      </button>
